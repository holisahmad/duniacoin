...
function confirmation(noid, kat, page) {
	...
	if (answer){
		//alert("Bye bye!")
		window.location = "?m=lihat_tf_admin&page=" + kat + "&no=" + noid;
	}
}
</script>
<?
...
if($kat == 1) {
	$sql0 = $db->count_records("gh_start", "penampungan=1 $aktif");
	$sql = $db->select("*", "gh_start", "penampungan=1 $aktif", "", "", "", "$start, $limit");
	//mysql_close();
} else if($kat == 2) {
	...
	$sql0 = $db->count_records("gh_start", "penampungan=1 $aktif");
	$sql = $db->select("*", "gh_start", "penampungan=1 $aktif", "id desc", "", "", "$start, $limit");
} else {
	$sql0 = $db->count_records("gh_start", "penampungan=1");
	$sql = $db->select("*", "gh_start", "penampungan=1", "id desc", "", "", "$start, $limit");
}					
	$numrows = $sql0;
	
?>
<p align="center"><strong>DAFTAR TAKE FUN KHUSUS MILIK ADMIN (TF)</strong></p>
<table width="98%" border="1" align="center" cellpadding="5" cellspacing="1" style="border:solid #CCCCCC 1px">
  <tr class="tbl_header">
    <td colspan="2">&nbsp;</td>
    <td colspan="9"><form id="form2" name="form2" method="post" action="?m=lihat_tf_admin&amp;kat=2" style="margin:0; padding:0">
          <label>
  ...
  <tr class="tbl_header">
    ...
    <td width="10%" align="center" bgcolor="#CCCCCC">NOMINAL TF</td>
    <td width="11%" align="center" bgcolor="#CCCCCC">DONATUR-1</td>
    <td width="11%" align="center" bgcolor="#CCCCCC">JUMLAH  1</td>
    <td width="13%" align="center" bgcolor="#CCCCCC">TERBAYAR</td>
    <td width="15%" align="center" bgcolor="#CCCCCC">KEKURANGAN</td>
...
?>  
  <tr class="<?= $class; ?>">
    ...
    <td align="center"><a href="?m=lihat_tf_admin&amp;page=edit&noid=<?= $row[0]; ?>"><img src="images/edit_f2.png" title="Edit GH" width="17" height="22" border="0" /></a></td>
...
?>
  <a href="?m=lihat_tf_admin&kat=<?= $kat; ?>&show=1" style="font-size:10px; color:#0000CC"><< First </a> | <a href="?m=lihat_tf_admin&kat=<?= $kat; ?>&show=<?= $previous; ?>" style="font-size:10px; color:#0000CC">< Previous </a> |
  <?php
...
?>
[ <a href="?m=lihat_tf_admin&kat=<?= $kat; ?>&show=<?= $i; ?>" style="font-size:10px; color:#0000CC">
<?= $i; ?>
</a> ]
<?php
...
?>
| <a href="?m=lihat_tf_admin&kat=<?= $kat; ?>&show=<?= $next; ?>" style="font-size:10px; color:#0000CC">Next ></a> | <a href="?m=lihat_tf_admin&kat=<?= $kat; ?>&show=<?= $paging; ?>" style="font-size:10px; color:#0000CC">Last >></a>
<?php
...
?>
  <?
	...
	case delete :
		...
		echo "<meta http-equiv='refresh' content='0;URL=?m=lihat_tf_admin'>";
	break;
...
if( isset($_POST['submit'])) {
	...
	echo "<meta http-equiv=\"refresh\" content=\"2; url=?m=lihat_tf_admin\">";
}
