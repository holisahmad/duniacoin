...
function confirmation(mid, page, action) {
	...
	if (answer){
		//alert("Bye bye!")
		window.location = "?m=withdrawlbulanan";
...
<h2 align="center"><img src="images/icon-48-user.png" width="48" height="48" align="absmiddle" /> Pembayaran Komisi AM/RO</h2>
...
if($kat == "") {
	...
	$db->select("a.id, a.userid, a.tglminta, a.tglbayar, a.rp, a.adm, a.automaintain, a.jmltrf, a.valid, a.status, a.tujuan, b.nama, b.tgl_daftar, b.bank, b.norek, b.an", "transferro as a inner join member as b on a.userid=b.username", $wherekom, "a.id DESC", "", "", "$start, $limit");

} else {
	//$numrows = $db->count_records("transfer", $where);	
	$db->select("a.id, a.userid, a.tglminta, a.tglbayar, a.rp, a.adm, a.automaintain, a.jmltrf, a.valid, a.status, a.tujuan, b.nama, b.tgl_daftar, b.bank, b.norek, b.an", "transferro as a inner join member as b on a.userid=b.username", $where, "a.id DESC", "", "", "$start, $limit");

}
$sel = "selected";
?>
<?
$sql1 = mysql_query("select rp, adm, jmltrf from transferro where kode='$kode'");
...
<table width="95%" border="0" align="center" cellpadding="5" cellspacing="0">
      ...
      <tr>
        <td width="100%"><form action="" method="post" name="form1" id="form1" style="margin:0; padding:0">
          <label> Status Withdrawl :
            ...
            <option value="?m=withdrawlbulanan" selected="selected" <? echo $pilih; ?>>Semua</option>
            <option value="?m=withdrawlbulanan&amp;kat=0&amp;sel2=<?= $sel; ?>" <? echo $sel2; ?>>Pending</option>
            <option value="?m=withdrawlbulanan&amp;kat=1&amp;sel3=<?= $sel; ?>" <? echo $sel3; ?>>Done</option>
          </select>
          </label>
          &nbsp;&nbsp;
        </form></td>
  ...
  <tr class="tbl_header">
	...
	<td width="8%" align="center">Automaintain</td>
...
for($i=0;$i<$j;$i++) {
	...
	if( $db->result($i, "status") > 0) {
		...
		$aktif = "<form name='form3' method='post' action='?m=withdrawlbulanan&page=proses&tombol=konfirm'>"
...
$total0 = $db->totalkomisibulanan( $user, "AND (tglbayar BETWEEN '$tgaktif' AND '".$db->result($i, "tglbayar")."') and status=0");

$widrl = $db->bayarkomisibulanan($user, "AND (tglbayar BETWEEN '$tgaktif' AND '".$db->result($i, "tglbayar")."')");
$total0 = $total0 - $widrl;
...
?>
  ...
  <tr class="<?= $class; ?>">
    ...
    <td align="right">
      <div align="right">
        <?= rupiah($db->result($i, "rp")); ?>
    </div></td>
    <td align="right"><?= rupiah($db->result($i, "automaintain")); ?></td>
	<td align="right"><?= rupiah($db->result($i, "adm")); ?></td>
    <td align="right"><?= rupiah($db->result($i, "jmltrf")); ?></td>
...
?>
  <a href="?m=withdrawlbulanan&kat=<?= $kat; ?>&show=1" style="font-size:10px; color:#0000CC"><< Awal </a> | <a href="?m=withdrawlbulanan&kat=<?= $kat; ?>&show=<?= $previous; ?>" style="font-size:10px; color:#0000CC">< Sebelumnya </a> |
  <?php
...
?>
[ <a href="?m=withdrawlbulanan&kat=<?= $kat; ?>&show=<?= $i; ?>" style="font-size:10px; color:#0000CC">
<?= $i; ?>
</a> ]
<?php
...
?>
| <a href="?m=withdrawlbulanan&kat=<?= $kat; ?>&show=<?= $next; ?>" style="font-size:10px; color:#0000CC">Selanjutnya ></a> | <a href="?m=withdrawlbulanan&kat=<?= $kat; ?>&show=<?= $paging; ?>" style="font-size:10px; color:#0000CC">Terakhir >></a>
<?php
...
if($tombol == "proses") {
	...
	$time = (date("H:i:s"));
	$uraian = "transfer komisi AM/RO untuk $mid via $tujuan";
	$adm=5000;
	$jmltrf=$jmltransfer;
	if($w == 0) {
			$db->insert("transferro", "", "'', '$mid', '$tgbyr $time', '$tgbyr $time', '$jumlah', $adm, '', $jmltrf, 1, 0,'$tujuan', '$kode'");
	...
	$db->insert("history_cardro", "", "'', '$mid', '1', 'Aktivasi Tiket RO Dari Automantent', '$clientdate'");
		...
		$admbank=5000;
		$jumlahe=$jmltrf;
		$db->insert("bayartoday", "", "'', '$mid', '$nama', '$bank', '$norek', '$an', '', '$admbank', '$jumlahe',  '$clientdate', 1, 0");
		}
		
	} else {
		$db->update("transferro", "status=1, tglbayar='$tgbyr $time', kode='$kode'", "id='$uid'");
...
<div id="notice"><img src="images/notice-info.png" width="29" height="29" align="absmiddle" />Withdrawl member telah berhasil dicatat ke dalam database! </div>
    <p align="center"><a href="?m=withdrawlbulanan">Kembali ke daftar Withdrawl</a></p>
...
if($tombol == "konfirm") {
	...
	$total0 = $db->totalkomisibulanan( $mid, "and status=0");
	$widrl = $db->bayarkomisibulanan($mid, "");
	$tarik = $total0 - $widrl;
?>    

<form id="form3" name="form2" method="post" action="?m=withdrawlbulanan&amp;page=proses">
...
?>
<form id="form2" name="form2" method="post" action="?m=withdrawlbulanan&page=proses">
  <table width="80%" border="0" align="center" cellpadding="3" cellspacing="0">
    <tr>
      <td width="46%" align="right">ID Member :</td>
      <td width="54%"><strong>
