...
<div id="menu_button">
  <ul>
    <li><a href="?m=content&page=addnew">Add New Article</a></li>
  </ul>
</div>
...
for($i=0;$i<$j;$i++) {
	...
	if($db->result($i, "published") > 0) {
		$img = "<a href='?m=content&page=publish&pub=0&no=".$db->result($i, "id")."'><img src='images/tick.png' border=0 title='Click to Unpublish'></a>";
	} else {
		$img = "<a href='?m=content&page=publish&pub=1&no=".$db->result($i, "id")."'><img src='images/publish_x.png' border=0 title='Click to Publish'></a>";
	} 	
	//---frontpage
	if($db->result($i, "frontpage") > 0) {
		$frontpage = "<a href='?m=content&page=frontpage&pub=0&no=".$db->result($i, "id")."'><img src='images/tick.png' border=0 title='Click to Unpublish'></a>";
	} else {
		$frontpage = "<a href='?m=content&page=frontpage&pub=1&no=".$db->result($i, "id")."'><img src='images/publish_x.png' border=0 title='Click to Publish'></a>";
	} 
?>
  ...
  <tr class="<?= $class; ?>">
    ...
    <td><a href="?m=content&page=addnew&edit=1&no=<?= $db->result($i, "id"); ?>"><?= $db->result($i, "title"); ?></a></td>
...
?>
<form action="?m=content&page=submit" method="post" name="konten">
...
<?	
	...
	case submit :
		...
		if($edit > 0) {
			$db->update("content", "title='$title', created='$crdate', created_by_alias='$author', maintext='$variable', published='$publish'", "id='$no'");
	} else {
