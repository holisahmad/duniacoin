...
if($kat == 1) {
	$sql0 = $db->count_records("gh_start", "status=1 $aktif");
	$sql = $db->select("*", "gh_start", "status=1 $aktif", "", "", "", "$start, $limit");
	//mysql_close();
} else if($kat == 2) {
	...
	$sql0 = $db->count_records("gh_start", "status=1 $aktif");
	$sql = $db->select("*", "gh_start", "status=1 $aktif", "id desc", "", "", "$start, $limit");
} else {
	$sql0 = $db->count_records("gh_start", "status=1");
	$sql = $db->select("*", "gh_start", "status=1", "", "", "", "$start, $limit");
}					
	$numrows = $sql0;
