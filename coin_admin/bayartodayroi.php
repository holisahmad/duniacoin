...
function confirmation(mid, page, action) {
	...
	if (answer){
		...
		window.location = "?m=bayartodayroi&page=" + page + "&mid=" + mid + "&action=" + action;
...
<h2 align="center"><img src="images/edit_f2.png" width="32" height="32" align="absmiddle" /> Rekap Pembayaran ROI</h2>
...
if($kat == "") {
	...
	$numrows = $db->count_records("bayartodayroi", "status=$kat");	

	$db->select("id, username, tanggal, nama, bank, norek, an, saldo, adm, pajak, bonus, uraian, status", "bayartodayroi", "status=0", $order, "", "", "$start, $limit");
...
<form action="?m=bayartodayroi&amp;page=konfirm" method="post"  name="bayar">
  ...
  <tr class="tbl_header">
    ...
    <td width="5" align="center"> Bank </td>
...
?>
  ...
  <a href="../admin/?m=bayartodayroi&amp;kat=<?= $kat; ?>&amp;show=1" style="font-size:10px; color:#0000CC"><< Awal </a> | <a href="../admin/?m=bayartodayroi&amp;kat=<?= $kat; ?>&amp;show=<?= $previous; ?>" style="font-size:10px; color:#0000CC">< Sebelumnya </a> |
...
[ <a href="../admin/?m=bayartodayroi&amp;kat=<?= $kat; ?>&amp;show=<?= $i; ?>" style="font-size:10px; color:#0000CC">
...
| <a href="../admin/?m=bayartodayroi&amp;kat=<?= $kat; ?>&amp;show=<?= $next; ?>" style="font-size:10px; color:#0000CC">Selanjutnya ></a> | <a href="../admin/?m=bayartodayroi&amp;kat=<?= $kat; ?>&amp;show=<?= $paging; ?>" style="font-size:10px; color:#0000CC">Terakhir >></a>
...
<?
	...
	case activation :
		if($action == "Disable") {
		$db->update("bayartodayroi", "status=0", "id='$mid'");
		echo "<meta http-equiv='refresh' content='0;URL=?m=bayartodayroi'>";
		} else { 
		$db->update("bayartodayroi", "status=1", "id='$mid'");
		
		$sql = mysql_query("select id, username, saldo , bonus , uraian from bayartodayroi where id='$mid'");
		...
		$sqlx = mysql_query("select id, userid  from transferroi where bonus='$bayar0' and userid ='$userx' and valid=0");
		$dtax = mysql_fetch_row($sqlx);
		$idx = $dtax[0];
		
		
		$db->update("transferroi", "valid=1", "id='$idx'");
...
$message="Management Dunia Coin Mengucapkan Selamat Hari ini ".$hariini."  ".$uraian." Sebesar ".$bayarx." Sukses Untuk Kita Bersama" ;
...
if( isset($_POST['submit'])) {
	$id[] = array();
  		for($i=0;$i<$j;$i++) {
		$db->update("bayartodayroi", "status=1" , "id='$id[$i]'");
		//	$db->delete("komisi", "id='$id[$i]'");
		//echo "$id[$i] id ";
		
		$sql = mysql_query("select id, username, saldo , bonus , uraian from bayartoday where id='$id[$i]'");
		$dta = mysql_fetch_row($sql);
		$userx= $dta[1];
		$bayar0 = $dta[2];
		$bonus = $dta[3];
...
//$db->smsnotifikasi ($hp , $message) ;
	...
	?>
<form action="" method="post" enctype="?m=bayartodayroi&amp;page=konfirm" name="bayar" id="bayar">
