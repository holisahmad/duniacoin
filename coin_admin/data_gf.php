...
function confirmation(noid, kat, page) {
	...
	if (answer){
		//alert("Bye bye!")
		window.location = "?m=data_gf&page=" + kat + "&no=" + noid;
	}
}
</script>
<?
...
if($kat == 1) {
	$sql0 = $db->count_records("gf_start", "status=1 $aktif");
	$sql = $db->select("*", "gf_start", "status=1 $aktif", "", "", "", "$start, $limit");
	//mysql_close();
} else if($kat == 2) {
	...
	$sql0 = $db->count_records("gf_start", "status=0 $aktif");
	$sql = $db->select("*", "gf_start", "status=0 $aktif", "id desc", "", "", "$start, $limit");
} else {
	$sql0 = $db->count_records("gf_start", $pasif);
	$sql = $db->select("*", "gf_start", $pasif, "id desc", "", "", "$start, $limit");
}					
	$numrows = $sql0;
	
?>
<p align="center"><strong>DAFTAR GIVE FUND (GF)</strong></p>
<table width="98%" border="0" align="center" cellpadding="5" cellspacing="1" style="border:solid #CCCCCC 1px">
  <tr class="tbl_header">
    <td colspan="2">&nbsp;</td>
    <td colspan="10"><form id="form2" name="form2" method="post" action="?m=data_gf&amp;kat=2" style="margin:0; padding:0">
          <label>
  ...
  <tr class="tbl_header">
    ...
    <td width="13%" align="center" bgcolor="#CCCCCC">TANGGAL</td>
    <td width="10%" align="center" bgcolor="#CCCCCC">NOMINAL GF</td>
    <td width="10%" align="center" bgcolor="#CCCCCC">PAKET GF</td>
    <td width="10%" align="center" bgcolor="#CCCCCC">PENERIMA-1</td>
    <td width="11%" align="center" bgcolor="#CCCCCC">NOMINAL-1</td>
    <td width="9%" align="center" bgcolor="#CCCCCC">Deadline Pemasangan PH</td>
    <td width="7%" align="center" bgcolor="#CCCCCC">Harus Ganti PH</td>
    <td width="5%" align="center" bgcolor="#CCCCCC">Admin Proses</td>
    <td width="8%" align="center" bgcolor="#CCCCCC">Pasangkan Manual</td>
    <td width="5%" align="center" bgcolor="#CCCCCC">Status</td>
  </tr>
<?
...
$hari_ini=date("Y-m-d");
	...
	if($row[35] == 1) {
		$status_gf = "<img src='images/icon-16-checkin.png' title='GF Sukses' border=0 />";
	} else if($row[35] == 2) {
	$status_gf = "<img src='images/notice-alert-01.png' title='GF Batal' border=0 />";
	} else {
		$status_gf = "<img src='images/publish_x.png' title='GF Pending' border=0 />";
	}
	...
	if($row[25] == 2) {
		$gantiph = "<strong><blink><font color='#FF0000'><a href='?m=data_gf&amp;page=gantiphbaru&id=$row[0]'>PH Batal dan Harus Di Ganti PH Baru</a></font></blink></strong>";
	} else if($row[25]==3) {
		$gantiph = "<strong><blink><font color='#FF0000'>PH Batal Sudah Di Ganti Baru</font></blink></strong>";
		} else {
		$gantiph = "-";
	}
...
?>  
  <tr class="<?= $class; ?>">
    ...
    <td align="center"><?= $row[34]; ?></td>
    <td align="center"><?= $row[5]; ?></td>
    <td align="center"><?= rupiah($row[6]); ?></td>
    <td align="center"><?= $pasangkan; ?></td>
    <td align="center"><?= $gantiph; ?></td>
    <td align="center"><?= $aktif; ?></td>
    <td align="center"><a href="?m=data_gf&amp;page=edit&noid=<?= $row[0]; ?>"><img src="images/edit_f2.png" title="Edit GF" width="17" height="22" border="0" /></a></td>
...
?>
  <a href="?m=data_gf&kat=<?= $kat; ?>&show=1" style="font-size:10px; color:#0000CC"><< First </a> | <a href="?m=data_gf&kat=<?= $kat; ?>&show=<?= $previous; ?>" style="font-size:10px; color:#0000CC">< Previous </a> |
  <?php
...
?>
[ <a href="?m=data_gf&kat=<?= $kat; ?>&show=<?= $i; ?>" style="font-size:10px; color:#0000CC">
<?= $i; ?>
</a> ]
<?php
...
?>
| <a href="?m=data_gf&kat=<?= $kat; ?>&show=<?= $next; ?>" style="font-size:10px; color:#0000CC">Next ></a> | <a href="?m=data_gf&kat=<?= $kat; ?>&show=<?= $paging; ?>" style="font-size:10px; color:#0000CC">Last >></a>
<?php
...
?>
  <?
	...
	case delete :
		...
		echo "<meta http-equiv='refresh' content='0;URL=?m=data_gf'>";
	break;
...
echo "<center><h2>Maaf Jumlah Pemberi Bantuan Tidak Sesuai Dengan Jumlah yang Anda Masukan</h2></center>";
	...
	if($kepada1<>""){
	$db->insert("gf_proses", "", "'', '$trx', '$username', '$clientdate', '$kepada1', '$nominal1', '0', '0'");
	...
	if($kepada2<>""){
	$db->insert("gf_proses", "", "'', '$trx', '$username', '$clientdate', '$kepada2', '$nominal2', '0', '0'");
	$db->update("gh_start", "status='2'", "username='$kepada2' and status='0'");
	} else {
	}
	
	if($kepada3<>""){
	$db->insert("gf_proses", "", "'', '$trx', '$username', '$clientdate', '$kepada3', '$nominal3', '0', '0'");
	$db->update("gh_start", "status='2'", "username='$kepada3' and status='0'");
	} else {
	}
	
	if($kepada4<>""){
	$db->insert("gf_proses", "", "'', '$trx', '$username', '$clientdate', '$kepada4', '$nominal4', '0', '0'");
	$db->update("gh_start", "status='2'", "username='$kepada4' and status='0'");
	} else {
	}
	
	if($kepada5<>""){
	$db->insert("gf_proses", "", "'', '$trx', '$username', '$clientdate', '$kepada5', '$nominal5', '0', '0'");
	$db->update("gh_start", "status='2'", "username='$kepada5' and status='0'");
	} else {
	}
	
	if($kepada6<>""){
	$db->insert("gf_proses", "", "'', '$trx', '$username', '$clientdate', '$kepada6', '$nominal6', '0', '0'");
	$db->update("gh_start", "status='2'", "username='$kepada6' and status='0'");
	} else {
	}
	
	if($kepada7<>""){
	$db->insert("gf_proses", "", "'', '$trx', '$username', '$clientdate', '$kepada7', '$nominal7', '0', '0'");
	$db->update("gh_start", "status='2'", "username='$kepada7' and status='0'");
	} else {
	}
	
	if($kepada8<>""){
	$db->insert("gf_proses", "", "'', '$trx', '$username', '$clientdate', '$kepada8', '$nominal8', '0', '0'");
	$db->update("gh_start", "status='2'", "username='$kepada8' and status='0'");
	} else {
	}
	
	if($kepada9<>""){
	$db->insert("gf_proses", "", "'', '$trx', '$username', '$clientdate', '$kepada9', '$nominal9', '0', '0'");
	$db->update("gh_start", "status='2'", "username='$kepada9' and status='0'");
	} else {
	}
	
	if($kepada10<>""){
	$db->insert("gf_proses", "", "'', '$trx', '$username', '$clientdate', '$kepada10', '$nominal10', '0', '0'");
	...
	echo "<meta http-equiv=\"refresh\" content=\"2; url=?m=data_gf\">";
}
}
...
<form id="form2" name="form1" method="post" action="">
  ...
  <table width="80%" border="0" align="center" cellpadding="4" cellspacing="0">
    <tr>
      <td colspan="2" align="center"><p>Daftar Get Help (Peminta Bantuan)</p>
            ...
            <td><table width="100%" border="0" align="center" cellpadding="0" cellspacing="0" bgcolor="#E7E5D9">
              ...
              <tr>
                <td bgcolor="#FFFFFF"><table width="100%" border="0" cellpadding="1" cellspacing="1" bgcolor="#CCCCCC">
                  <tr>
                    <td width="6%" height="25" align="center" bgcolor="#F2F1EC"><font size="2" face="Verdana, Arial, Helvetica, sans-serif">No.</font></td>
                    <td width="21%" align="center" bgcolor="#F2F1EC"><font size="2" face="Verdana, Arial, Helvetica, sans-serif">Tanggal GH</font></td>
                    <td width="19%" align="center" bgcolor="#F2F1EC"><font size="2" face="Verdana, Arial, Helvetica, sans-serif">Username</font></td>
                    <td width="26%" align="center" bgcolor="#F2F1EC"><font size="2" face="Verdana, Arial, Helvetica, sans-serif">Nominal GH</font></td>
                    <td width="28%" align="center" bgcolor="#F2F1EC"><font size="2" face="Verdana, Arial, Helvetica, sans-serif">Terbayar</font></td>
                    <td width="28%" align="center" bgcolor="#F2F1EC"><font size="2" face="Verdana, Arial, Helvetica, sans-serif">Kekurangan</font></td>
                  </tr>
                  <?php
