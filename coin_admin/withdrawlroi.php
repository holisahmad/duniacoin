...
function confirmation(mid, page, action) {
	...
	if (answer){
		//alert("Bye bye!")
		window.location = "?m=withdrawlroi";
...
<h2 align="center"><img src="images/icon-48-user.png" width="48" height="48" align="absmiddle" /> Pembayaran ROI</h2>
...
if($kat == "") {
	...
	$db->select("a.id, a.userid, a.tglbayar, a.bank, a.an, a.norek, a.bonus, . a.adm, a.pajak, a.rp, a.status, a.valid, a.tujuan, b.nama", "transferroi as a inner join member as b on a.userid=b.username", $wherekom, "a.id DESC", "", "", "$start, $limit");

} else {
	//$numrows = $db->count_records("transfer", $where);	
	$db->select("a.id, a.userid, a.tglbayar, a.bank, a.an, a.norek, a.bonus, . a.adm, a.pajak, a.rp, a.status, a.valid, a.tujuan, b.nama", "transferroi as a inner join member as b on a.userid=b.username", $where, "a.id DESC", "", "", "$start, $limit");
...
<table width="95%" border="0" align="center" cellpadding="5" cellspacing="0">
      ...
      <tr>
        <td width="100%"><form action="" method="post" name="form1" id="form1" style="margin:0; padding:0">
          <label> Status Withdrawl :
            ...
            <option value="?m=withdrawlroi" selected="selected" <? echo $pilih; ?>>Semua</option>
            <option value="?m=withdrawlroi&amp;kat=0&amp;sel2=<?= $sel; ?>" <? echo $sel2; ?>>Pending</option>
            <option value="?m=withdrawlroi&amp;kat=1&amp;sel3=<?= $sel; ?>" <? echo $sel3; ?>>Done</option>
          </select>
          </label>
          &nbsp;&nbsp;
        </form></td>
  ...
  <tr class="tbl_header">
    ...
    <td width="12%" align="center">Data Rekening </td>
    
    <td width="19%" align="center">Keterangan </td>
...
for($i=0;$i<$j;$i++) {
	...
	if( $db->result($i, "status") > 0) {
		...
		$aktif = "<form name='form3' method='post' action='?m=withdrawlroi&page=proses&tombol=konfirm'>"
...
?>
  ...
  <tr class="<?= $class; ?>">
    ...
    <td align="center"><?= $db->result($i, "tujuan"); ?></td>
...
?>
  <a href="?m=withdrawlroi&kat=<?= $kat; ?>&show=1" style="font-size:10px; color:#0000CC"><< Awal </a> | <a href="?m=withdrawlroi&kat=<?= $kat; ?>&show=<?= $previous; ?>" style="font-size:10px; color:#0000CC">< Sebelumnya </a> |
  <?php
...
?>
[ <a href="?m=withdrawlroi&kat=<?= $kat; ?>&show=<?= $i; ?>" style="font-size:10px; color:#0000CC">
<?= $i; ?>
</a> ]
<?php
...
?>
| <a href="?m=withdrawlroi&kat=<?= $kat; ?>&show=<?= $next; ?>" style="font-size:10px; color:#0000CC">Selanjutnya ></a> | <a href="?m=withdrawlroi&kat=<?= $kat; ?>&show=<?= $paging; ?>" style="font-size:10px; color:#0000CC">Terakhir >></a>
<?php
...
<div id="notice"><img src="images/notice-info.png" width="29" height="29" align="absmiddle" />Withdrawl member telah berhasil dicatat ke dalam database! </div>
    <p align="center"><a href="?m=withdrawlroi">Kembali ke daftar Withdrawl</a></p>
...
<form id="form3" name="form2" method="post" action="?m=withdrawlroi&amp;page=proses">
...
?>
<form id="form2" name="form2" method="post" action="?m=withdrawlroi&page=proses">
  <table width="80%" border="0" align="center" cellpadding="3" cellspacing="0">
    <tr>
      <td width="46%" align="right">ID Member :</td>
      <td width="54%"><strong>
