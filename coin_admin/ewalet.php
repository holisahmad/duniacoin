...
$dtgl=$tanggal ;
$tanggal = "(tanggal between '$dtfrom' and '$dtto')";
switch($page) {
	default :
	//---pagination----------------
...
//$db->select("*", "member", $kat);
	$numrows = $db->count_records("automaintain", $tanggal);	
	$db->select("id, username, tanggal, total", "automaintain", $tanggal, "id desc", "", "", "$start, $limit");
...
?>
				...
				<tr>
				  <td><div class="ewalet">
				    ...
				    <h3 align="center">Catatan Saldo Automainten Member </h3>
                    </p>
				  </div></td>
...
<table width="96%" border="0" align="center" cellpadding="5" cellspacing="0">
  <tr class="tbl_header">
    ...
    <td width="28%" align="center">ID Member </td>
    <td width="36%" align="center">Jumlah  </td>
  </tr>
  <?
...
?>
  <tr class="<?= $class; ?>">
    <td width="14%" align="center"><?= $nom; ?>    </td>
    <td width="22%" align="center"><?= date("d-m-Y", strtotime($db->result($i, "tanggal"))); ?></td>
    <td align="center"><?= $user; ?></td>
    <td align="right"><?= rupiah($db->result($i, "total")); ?></td>
  </tr>
  <?
...
?>
  <a href="?m=saldoauto&kat=<?= $kat; ?>&show=1" style="font-size:10px; color:#0000CC"><< Awal </a> | <a href="?m=saldoauto&kat=<?= $kat; ?>&show=<?= $previous; ?>" style="font-size:10px; color:#0000CC">< Sebelumnya </a> |
  <?php
...
?>
[ <a href="?m=saldoauto&kat=<?= $kat; ?>&show=<?= $i; ?>" style="font-size:10px; color:#0000CC">
<?= $i; ?>
</a> ]
<?php
...
?>
| <a href="?m=saldoauto&kat=<?= $kat; ?>&show=<?= $next; ?>" style="font-size:10px; color:#0000CC">Selanjutnya ></a> | <a href="?m=saldoauto&kat=<?= $kat; ?>&show=<?= $paging; ?>" style="font-size:10px; color:#0000CC">Terakhir >></a>
<?php
...
//$db->select("*", "member", $kat);
	$numrows = $db->count_records("komisi", $tanggal);	
	$db->select("id, username, bayar, tglbayar, jenis, dari", "komisi", "bayar > 0 and username <>''", "id desc", "", "", "$start, $limit");
...
<form action="?m=saldoauto&amp;page=konfirm" method="post"  name="komisi">
...
?>
  <tr class="<?= $class; ?>">
    ...
    <td align="right"><?= rupiah($db->result($i, "bayar")); ?></td>
  </tr>
...
?>
  <a href="?m=saldoauto&page=komisi&show=1" style="font-size:10px; color:#0000CC"><< Awal </a> | <a href="?m=saldoauto&page=komisi&show=<?= $previous; ?>" style="font-size:10px; color:#0000CC">< Sebelumnya </a> |
  <?php
...
?>
[ <a href="?m=saldoauto&page=komisi&show=<?= $i; ?>" style="font-size:10px; color:#0000CC">
<?= $i; ?>
</a> ]
<?php
...
?>
| <a href="?m=saldoauto&page=komisi&show=<?= $next; ?>" style="font-size:10px; color:#0000CC">Selanjutnya ></a> | <a href="?m=saldoauto&page=komisi&show=<?= $paging; ?>" style="font-size:10px; color:#0000CC">Terakhir >></a>
<?php
  	...
  	if( isset($_POST['submit'])) {
		$id[] = array();
  		for($i=0;$i<$j;$i++) {
			...
			echo "<meta http-equiv='refresh' content='0;URL=?m=saldoauto&page=komisi'>";
			//echo "$id[$i] id ";
		}
	} else {		
	?>
<form action="" method="post" enctype="?m=saldoauto&amp;page=konfirm" name="komisi" id="komisi">
      <table width="100%" border="0" align="center" cellpadding="5" cellspacing="0">
        <tr class="tbl_header">
...
?>
        <tr>
          ...
          &nbsp;<input name="submit22" type="button" id="submit22" value="BATAL" class="tombol" onClick="javascript:saldoauto.go(-1)" /></td>
        </tr>
  </table>
</form>
	<p>&nbsp;</p>
