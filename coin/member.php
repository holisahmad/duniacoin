...
if($kat == 3) {
	...
	$db->select("id, username, nama, hp, sponsor, tgl_daftar, upline, kota, paket_daftar, status, blokir, negara", "member", "username like '%$keywrd%' or username like '%$keywrd%'", $order, "", "", "$start, $limit");
	
} else {
	$numrows = $db->count_records("member", "status=$kat and paket=1");	
	$db->select("id, username, nama,  hp, sponsor, tgl_daftar, upline, kota, paket_daftar,status, blokir , negara", "member", $where, $order, "", "", "$start, $limit");
}

if($kat == 1) {
$numrows = $db->count_records("member", "negara='Indonesia' and paket=1");	
	$db->select("id, username, nama,  hp, sponsor, tgl_daftar, upline, kota, paket_daftar,status, blokir , negara", "member", "negara='Indonesia'", $order, "", "", "$start, $limit");
}
if($kat == 2) {
$numrows = $db->count_records("member", "negara='Malaysia' and paket=1");	
	$db->select("id, username, nama,  hp, sponsor, tgl_daftar, upline, kota, paket_daftar,status, blokir , negara", "member", "negara='Malaysia'", $order, "", "", "$start, $limit");
}
    ...
    <td colspan="12" align="center" style="padding:0"><table width="100%" border="0" cellspacing="0" cellpadding="0" style="padding:0; margin:0">
	  ...
	  <td width="39%"><form action="" method="post" name="form1" id="form1" style="margin:0; padding:0">
          <label>  Negara:
            ...
            <option value="?e=member&amp;kat=1" >Member Indonesia</option>
            <option value="?e=member&amp;kat=2" >Member Malaysia</option>
          </select>
          </label>&nbsp;&nbsp;
        ...
        <td width="47%"><form id="form2" name="form2" method="post" action="?e=member&amp;kat=3" style="margin:0; padding:0">
          <label>
            Cari Member : 
...
?>
  ...
  <tr class="<?= $class; ?>">
    ...
    <td align="center"><a href="?e=member&page=addnew&edit=1&mid=<?= $db->result($i, "username"); ?>"><?= $db->result($i, "nama"); ?></a></td>
