...
function confirmation(noid, kat, page) {
	...
	if (answer){
		//alert("Bye bye!")
		window.location = "?e=data_gh&page=" + kat + "&no=" + noid;
	}
}
</script>
<?
...
if($kat == 1) {
	$sql0 = $db->count_records("gh_start", "negara='Indonesia' and status=1" );
	$sql = $db->select("*", "gh_start", "negara='Indonesia' and status=1" , "", "", "", "$start, $limit");
	//mysql_close();
} else if($kat == 2) {
	...
	$sql0 = $db->count_records("gh_start", "negara='Malaysia'  and status=1 ");
	$sql = $db->select("*", "gh_start", "negara='Malaysia'  and status=1", "id desc", "", "", "$start, $limit");
} else {
...
?>
<p align="center"><strong>DATA RA SUKSES (RA)</strong></p>
<table width="100%" border="0" cellspacing="0" cellpadding="5">
      ...
      <tr>
        <td><form action="" method="post" name="form1" id="form1" style="margin:0; padding:0">
          <label> Katagori Negara:
            ...
            <option value="?e=data_gh1" selected="selected" <? echo $pilih; ?>>Semua negara</option>
            <option value="?e=data_gh1&amp;kat=1&amp;sel3=<?= $sel; ?>" <? echo $sel3; ?>>Member Indonesia</option>
            <option value="?e=data_gh1&amp;kat=0&amp;sel2=<?= $sel; ?>" <? echo $sel2; ?>>Member Malaysia</option>
          </select>
          </label>&nbsp;&nbsp;
        Total: <b><?= $numrows; ?></b> orang
        </form></td>
        <td><form id="form2" name="form2" method="post" action="?e=data_gh&amp;kat=2" style="margin:0; padding:0">
          <label>
            Cari Member : 
  ...
  <tr bgcolor="#FF0000" class="tbl_header">
    ...
    <td width="7%" align="center">KODE TF</td>
    <td width="8%" align="center">NOMINAL TF</td>
    <td width="7%" align="center">DONATUR-1</td>
    <td width="8%" align="center">JUMLAH  1</td>
    <td width="12%" align="center">DONATUR-2</td>
    <td width="8%" align="center">JUMLAH  2</td>
    <td width="7%" align="center">TERBAYAR</td>
    <td width="10%" align="center">KEKURANGAN</td>
...
?>  
  <tr class="<?= $class; ?>">
    ...
    <td align="center"><a href="?e=data_gh&amp;page=edit&noid=<?= $row[0]; ?>"><img src="../admin/images/edit_f2.png" title="Edit GH" width="17" height="22" border="0" /></a></td>
...
?>
  <a href="?e=data_gh&kat=<?= $kat; ?>&show=1" style="font-size:10px; color:#0000CC"><< First </a> | <a href="?e=data_gh&kat=<?= $kat; ?>&show=<?= $previous; ?>" style="font-size:10px; color:#0000CC">< Previous </a> |
  <?php
...
?>
[ <a href="?e=data_gh&kat=<?= $kat; ?>&show=<?= $i; ?>" style="font-size:10px; color:#0000CC">
<?= $i; ?>
</a> ]
<?php
...
?>
| <a href="?e=data_gh&kat=<?= $kat; ?>&show=<?= $next; ?>" style="font-size:10px; color:#0000CC">Next ></a> | <a href="?e=data_gh&kat=<?= $kat; ?>&show=<?= $paging; ?>" style="font-size:10px; color:#0000CC">Last >></a>
<?php
...
?>
  <?
	...
	case delete :
		...
		echo "<meta http-equiv='refresh' content='0;URL=?e=data_gh'>";
	break;
...
if( isset($_POST['submit'])) {
	...
	echo "<meta http-equiv=\"refresh\" content=\"2; url=?e=data_gh\">";
}
