...
function confirmation(noid, kat, page) {
	...
	if (answer){
		//alert("Bye bye!")
		window.location = "?e=data_pa&page=" + kat + "&no=" + noid;
	}
}
</script>
<?
...
if($kat == 1) {
	$sql0 = $db->count_records("pa_start", "negara='Indonesia' $aktif");
	$sql = $db->select("*", "pa_start", "negara='Indonesia' $aktif","id desc", "", "", "$start, $limit");
	//mysql_close();
} else if($kat == 2) {
	$sql0 = $db->count_records("pa_start", "username like '%$keywrd%' or username like '%$keywrd%'");
	$sql = $db->select("*", "pa_start", "username like '%$keywrd%' or username like '%$keywrd%'", "id desc", "", "", "$start, $limit");
} else if($kat == "0") {
	$sql0 = $db->count_records("pa_start", "negara='Malaysia' $aktif");
	$sql = $db->select("*", "pa_start", "negara='Malaysia' $aktif", "id desc", "", "", "$start, $limit");
} else {
	$sql0 = $db->count_records("pa_start", $pasif);
	$sql = $db->select("*", "pa_start", $pasif, "id desc", "", "", "$start, $limit");
}					
	$numrows = $sql0;
	$sel = "selected";
...
<table width="100%" border="0" cellspacing="0" cellpadding="5">
      ...
      <tr>
        <td><form action="" method="post" name="form1" id="form1" style="margin:0; padding:0">
          <label> Katagori Negara:
            ...
            <option value="?e=data_pa" selected="selected" <? echo $pilih; ?>>Semua negara</option>
            <option value="?e=data_pa&amp;kat=1&amp;sel3=<?= $sel; ?>" <? echo $sel3; ?>>Member Indonesia</option>
            <option value="?e=data_pa&amp;kat=0&amp;sel2=<?= $sel; ?>" <? echo $sel2; ?>>Member Malaysia</option>
          </select>
          </label>&nbsp;&nbsp;
        Total: <b><?= $numrows; ?></b> orang
        </form></td>
        <td><form id="form2" name="form2" method="post" action="?e=data_pa&amp;kat=2" style="margin:0; padding:0">
          <label>
            Cari Member : 
  ...
  <tr bgcolor="#FF0000" class="tbl_header">
    ...
    <td width="17%" align="center">NOMINAL </td>
    <td width="20%" align="center">ANTRIAN</td>
...
$hari_ini=date("Y-m-d");
	...
	if($row[35] == 1) {
		$status_pa = "<img src='../admin/images/icon-16-checkin.png' title='GF Sukses' border=0 />";
	} else if($row[35] == 2) {
	$status_pa = "<img src='../admin/images/notice-alert-01.png' title='GF Batal' border=0 />";
	} else {
		$status_pa = "<img src='../admin/images/publish_x.png' title='GF Pending' border=0 />";
	}
	...
	if($row[25] == 7) {
		$gantiph = "<strong><blink><font color='#FF0000'><a href='?e=data_pa&amp;page=gantiphbaru&id=$row[0]'>PA Batal dan Harus Di Ganti PA Baru</a></font></blink></strong>";
	} else if($row[25]==5) {
...
?>
  <a href="?e=data_pa&kat=<?= $kat; ?>&show=1" style="font-size:10px; color:#0000CC"><< First </a> | <a href="?e=data_pa&kat=<?= $kat; ?>&show=<?= $previous; ?>" style="font-size:10px; color:#0000CC">< Previous </a> |
  <?php
...
?>
[ <a href="?e=data_pa&kat=<?= $kat; ?>&show=<?= $i; ?>" style="font-size:10px; color:#0000CC">
<?= $i; ?>
</a> ]
<?php
...
?>
| <a href="?e=data_pa&kat=<?= $kat; ?>&show=<?= $next; ?>" style="font-size:10px; color:#0000CC">Next ></a> | <a href="?e=data_pa&kat=<?= $kat; ?>&show=<?= $paging; ?>" style="font-size:10px; color:#0000CC">Last >></a>
<?php
...
?>
  <?
	break;
	case delete :
		$db->delete("pa_start", "id='$no'");
		echo "<meta http-equiv='refresh' content='0;URL=?e=data_pa'>";
	break;
	...
	case gantiphbaru :
	$db->update("pa_start", "status='3'", "id='$id'");
		echo "<meta http-equiv='refresh' content='0;URL=?e=data_pa'>";
	break;
...
echo "<center><h2>Maaf Jumlah Pemberi Bantuan Tidak Sesuai Dengan Jumlah yang Anda Masukan</h2></center>";
	...
	echo "<meta http-equiv=\"refresh\" content=\"2; url=?e=data_pa\">";
}
}
	$db->select("*", "pa_start", "id='$noid'");	
?>
</p>
<p>&nbsp;</p>
<p></p>
