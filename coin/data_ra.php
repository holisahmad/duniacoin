...
function confirmation(noid, kat, page) {
	...
	if (answer){
		//alert("Bye bye!")
		window.location = "?e=data_ra&page=" + kat + "&no=" + noid;
	}
}
</script>
<?
...
if($kat == 1) {
	$sql0 = $db->count_records("ra_start", "negara='Indonesia' $aktif");
	$sql = $db->select("*", "ra_start", "negara='Indonesia' $aktif", "", "", "", "$start, $limit");
	//mysql_close();
} else if($kat == 2) {
	$sql0 = $db->count_records("ra_start", "username like '%$keywrd%' or username like '%$keywrd%'");
	$sql = $db->select("*", "ra_start", "username like '%$keywrd%' or username like '%$keywrd%'", "id desc", "", "", "$start, $limit");
} else if($kat == "0") {
	$sql0 = $db->count_records("ra_start", "negara='Malaysia' $aktif");
	$sql = $db->select("*", "ra_start", "negara='Malaysia' $aktif", "id desc", "", "", "$start, $limit");
} else {
	$sql0 = $db->count_records("ra_start", $pasif);
	$sql = $db->select("*", "ra_start", $pasif, "id desc", "", "", "$start, $limit");
}					
	$numrows = $sql0;
	$sel = "selected";
	
?>
<p align="center"><strong>DATA ANTRIAN  (RA)</strong></p>
<table width="100%" border="0" cellspacing="0" cellpadding="5">
      ...
      <tr>
        <td><form action="" method="post" name="form1" id="form1" style="margin:0; padding:0">
          <label> Katagori Negara:
            ...
            <option value="?e=data_ra" selected="selected" <? echo $pilih; ?>>Semua negara</option>
            <option value="?e=data_ra&amp;kat=1&amp;sel3=<?= $sel; ?>" <? echo $sel3; ?>>Member Indonesia</option>
            <option value="?e=data_ra&amp;kat=0&amp;sel2=<?= $sel; ?>" <? echo $sel2; ?>>Member Malaysia</option>
          </select>
          </label>&nbsp;&nbsp;
        Total: <b><?= $numrows; ?></b> orang
        </form></td>
        <td><form id="form2" name="form2" method="post" action="?e=data_ra&amp;kat=2" style="margin:0; padding:0">
          <label>
            Cari Member : 
  ...
  <tr bgcolor="#FF0000" class="tbl_header">
    ...
    <td width="7%" align="center">ANTRIAN</td>
    <td width="8%" align="center">NOMINAL </td>
    <td width="11%" align="center">DONATUR-1</td>
    <td width="8%" align="center">JUMLAH  1</td>
    <td width="11%" align="center">DONATUR-2</td>
    <td width="8%" align="center">JUMLAH  2</td>
    <td width="7%" align="center">TERBAYAR</td>
    <td width="11%" align="center">KEKURANGAN</td>
...
?>
  <a href="?e=data_ra&kat=<?= $kat; ?>&show=1" style="font-size:10px; color:#0000CC"><< First </a> | <a href="?e=data_ra&kat=<?= $kat; ?>&show=<?= $previous; ?>" style="font-size:10px; color:#0000CC">< Previous </a> |
  <?php
...
?>
[ <a href="?e=data_ra&kat=<?= $kat; ?>&show=<?= $i; ?>" style="font-size:10px; color:#0000CC">
<?= $i; ?>
</a> ]
<?php
...
?>
| <a href="?e=data_ra&kat=<?= $kat; ?>&show=<?= $next; ?>" style="font-size:10px; color:#0000CC">Next ></a> | <a href="?e=data_ra&kat=<?= $kat; ?>&show=<?= $paging; ?>" style="font-size:10px; color:#0000CC">Last >></a>
<?php
...
?>
  <?
	break;
	case delete :
		$db->delete("ra_start", "id='$no'");
		echo "<meta http-equiv='refresh' content='0;URL=?e=data_ra'>";
	break;
...
if( isset($_POST['submit'])) {
	$db->update("ra_start", "terbayar='$terbayar', kekurangan='$kekurangan'", "id='$noid'");
	echo "<p align=center><b>Data GF Berhasil Diupdate!</b></p>";
	echo "<meta http-equiv=\"refresh\" content=\"2; url=?e=data_ra\">";
}
	$db->select("*", "ra_start", "id='$noid'");	
?>
</p>
<p>&nbsp;</p>
<p></p>
