...
function confirmation(noid) {
	...
	if (answer){
		...
		window.location = "?e=konfir&page=delete&id=" + noid;
...
<h2 align="center"><img src="../admin/images/icon-48-user.png" width="48" height="48" align="absmiddle" /> Data Konfirmasi Transfer</h2>
...
$penerima = $db->result($i, "penerima");
$pengirim = $db->result($i, "pengirim");
...
for($i=0;$i<$j;$i++) {
	...
	if($db->result($i, "status") > 0) {
		...
		$img = "<a href='?e=konfir&page=publish&pub=0&id=".$db->result($i, "id")."'><img src='../admin/images/tick.png' border=0 title='Click to Unpublish'></a>";

	} else {

		$img = "<a href='?e=konfir&page=publish&pub=1&id=".$db->result($i, "id")."'><img src='../admin/images/publish_x.png' border=0 title='Click to Publish'></a>";
...
?>
  ...
  <tr class="<?= $class; ?>">
    ...
    <td valign="top"><div align="center"><a href="?e=konfir&page=addnew&edit=1&id=<?= $db->result($i, "id"); ?>">
...
?>
  ...
  <a href="?e=konfir&kat=<?= $kat; ?>&show=1" style="font-size:10px; color:#0000CC"><< Awal </a> | <a href="?e=konfir&kat=<?= $kat; ?>&show=<?= $previous; ?>" style="font-size:10px; color:#0000CC">< Sebelumnya </a> |
...
[ <a href="?e=konfir&kat=<?= $kat; ?>&show=<?= $i; ?>" style="font-size:10px; color:#0000CC">
...
| <a href="?e=konfir&kat=<?= $kat; ?>&show=<?= $next; ?>" style="font-size:10px; color:#0000CC">Selanjutnya ></a> | <a href="?e=konfir&kat=<?= $kat; ?>&show=<?= $paging; ?>" style="font-size:10px; color:#0000CC">Terakhir >></a>
...
<form action="?e=konfir&amp;page=submit" method="post" enctype="multipart/form-data">
...
$db->select("username", "gf_proses", "username='$pengirim' and bayar='0' and kepada='$penerima'");
$chkd_user = $db->num_rows();
	if ($chkd_user!= "") {
		$db->update("gf_proses", "bayar='1'", "username='$pengirim' and kepada='$penerima'");
	} else {
	}

$db->select("username", "gf_proses", "username='$pengirim' and bayar='0'");
	$chkd_user = $db->num_rows();
	if ($chkd_user!= "") {
	} else {
		$db->update("gf_start", "sukses='1'", "username='$pengirim' and trx='$trx'");
	}
			
		$db->update("komisi_sponsor", "validasi='1'", "jenis='Komisi Sponsor' and dari='$pengirim'");

		echo "<meta http-equiv='refresh' content='0;URL=?e=konfir'>";
